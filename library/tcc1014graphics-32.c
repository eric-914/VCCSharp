      ...
      for (unsigned short beam = 0; beam < gs->BytesperRow; beam += 2) //1bbp Stretch=2
      {
        ...
        { //Pcolor
          for (bit = 7; bit >= 0; bit--)
          {
            ...
            case 3:
              color = 3;
              szSurface32[yStride - 1] = graphicsColors->Afacts32[gs->ColorInvert][3];

              if (!emuState->ScanLines) {
                szSurface32[yStride + Xpitch - 1] = graphicsColors->Afacts32[gs->ColorInvert][3];
              }

              szSurface32[yStride] = graphicsColors->Afacts32[gs->ColorInvert][3];

              if (!emuState->ScanLines) {
                szSurface32[yStride + Xpitch] = graphicsColors->Afacts32[gs->ColorInvert][3];
              }
            ...
            szSurface32[yStride += 1] = graphicsColors->Afacts32[gs->ColorInvert][color];

            if (!emuState->ScanLines) {
              szSurface32[yStride + Xpitch] = graphicsColors->Afacts32[gs->ColorInvert][color];
            }
