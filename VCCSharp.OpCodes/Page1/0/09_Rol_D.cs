...
/// [        ↕ ↕ ↕ ↕]
/// 
/// The ROL instruction rotates the contents of the specified byte in memory to the left by one bit, through the Carry bit of the CC register (effectively a 9-bit rotation). 
...
internal class _09_Rol_D : OpCode, IOpCode
{
    ...
    public int Exec()
    {
        ...
        CC_N = result.Bit7();
        CC_Z = result == 0;
        CC_V = value.Bit7() ^ value.Bit6(); //((CC_C ? 1 : 0) ^ ((value & 0x40) >> 6)) != 0;
        CC_C = value.Bit7();
